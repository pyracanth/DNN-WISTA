function [network, Z, Zstar, ZT, info] = DNNihta_trainfull(D, X, Zorig, network, info, num_of_classes, learning_rate, max_epoch, conv_count_thres, error_check_iter )
...
[ZT,Zstar,info.convergeiter,info.Zerr,info.avgdiversity_hoyer]= ...
    ihta_fprop_errsparsity(Zorig,X,network.W,network.H,network.t,info,network.T, 500);
toc;
...
[~,Z,info.totallayer,info.Z1err,info.avgdiversity_hoyer1]=...
    ihta_fprop_errsparsity(Zorig,X,network.W,network.H,network.t,info,network.T,network.T1);
switch info.netlearn
    case 1
        err=Zstar-Z;
